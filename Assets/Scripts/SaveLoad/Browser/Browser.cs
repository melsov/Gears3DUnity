...
public class Browser : Singleton<Browser>
{
    ...
    public delegate void HandleNewProjectName(string _newName);
    public HandleNewProjectName handleNewProjectName;
    ...
    public void SaveFile(String FilePath, String mainFolder, HandleOpenFile _handleSaveFile) {
        handleSaveFile = _handleSaveFile;
        if (savePath != null) {
            handleSaveFile(savePath);
            return;
        } 
        else {
    ...
    //MMP
    private void saveToLocation(string _location) {
        savePath = _location;
        handleSaveFile(savePath);
    }
    ...
    void DrawBrowser(int windowID) {
        ...
        if (WinType == WindowType.Save && GUI.Button(bottom, "Save")) {
            ...
            if (Directory.Exists(SaveLocation)) {
                ...
                saveToLocation(SaveLocation + "/" + NameOfFileToSave);
            } else {
                // If the user chooses a file instead of a folder the parent folder will be used.
        ...
        if (WinType == WindowType.Save) {
            ...
            if (handleNewProjectName != null) {
                handleNewProjectName(NameOfFileToSave);
            }
        }
